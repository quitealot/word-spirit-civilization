...
## 0. 2026-09-01 当前方向（优先于下方历史产品定位）

用户确认《语灵》转向完全无英语学习关联的独立可安装游戏应用：立绘、语灵收集与成长、剧情探索、回合制；希望有商业收入，同时给玩家舒适、有趣、可耐心观察的体验。微信小游戏目标已于2026-09-01取消，最新目标是让美术、动作与特效充分发挥，即使上架手续更复杂也接受。
当前原生应用方向入口：`docs/NATIVE_APPLICATION_DIRECTION_V1.md`。Unity 6 + URP为推荐候选，尚未授权创建Unity工程或迁移现有代码。
...
现有网页只作历史原型与快速交互参考，不是正式客户端底座；尚不可直接迁移整套主线或宣称原生应用已开始实现。
