...
用户否决最近两张重新排版稿，要求在旧图基础局部优化。基准原件：`user-approved-battle-reference.png`，不可覆盖；当前局部示意：`reference-local-polish-v1.png`，等待用户确认。以后不从V1/V2新稿反推方向。

局部稿采用内置imagegen编辑（非CLI）：保留旧图的墨绿金配色、雾中遗迹、布局和上下HP条，移除英语UI，修澜歌方底，原底部区域换水音/回潮/静波。敌方立绘未定仍保留原占位，不新增正式敌人。图像编辑无法保证逐像素保留角色，细节有漂移；此图只看局部UI方向，不能替代原始立绘生产资产。静态图未验证回合动画，没有修改或部署运行代码。

提示词约束：直接编辑所附截图而非重做；保持900:642比例、构图、标题、背景、原角色设计和敌方占位；仅修小蓝色语灵方底及三者可读性，知识加成改你的回合，血条标签改敌方HP/我方HP，移除翻译中；消息条改选择语灵与技能；底部原答题区改澜歌及水音/回潮/静波，副标签压制/回复/防护，水音沿用淡金选中态；标注静态优化稿；禁止新增面板、货币、星级、捕捉、边框、背景、阳光或特效。
