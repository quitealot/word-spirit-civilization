...
V1.1 取消 `water → 水音`、`help → 回潮` 的固定所有权。玩家先选择技能，系统再从本场同时满足 `Used + battleEligible + 行动语义匹配` 的候选池中确定“本次调用词”。技能按钮不预先展示某个单词；调用词只在技能选择之后出现。当前两个技能与两个词在本场“语灵站支援行动”语境下形成多对多候选，但不代表永久绑定，也不改变全局技能或词汇规则。
