...
function HpBar({ enemy = false, hp, previous, max, phase }: { enemy?: boolean; hp: number; previous: number; max: number; phase: string }) {
  return <div className={`bu-hp ${enemy ? 'bu-hp-enemy' : 'bu-hp-player'} ${hp <= max * .25 ? 'bu-hp-low' : ''}`}>
    <div className="bu-avatar" aria-hidden="true">{enemy ? <span>魔</span> : <img src="/battle-ui/lange-cutout.png" alt="" />}</div>
    <div className="bu-hp-main">
      ...
      <div className="bu-hp-track" role="progressbar" aria-label={enemy ? '守门人生命' : '澜歌生命'} aria-valuemin={0} aria-valuemax={max} aria-valuenow={hp}>
        <div key={`${phase}-${hp}-trail`} className="bu-hp-trail" style={{ width: `${Math.max(previous, hp) / max * 100}%`, ...(hp < previous ? { animationName: 'bu-trail', '--bu-final': `${hp / max * 100}%` } : {}) }} />
...
export default function BattleUiPage() {
  ...
  const resultText = state.phase === 'player'
  ...
  return <main className="bu-shell">
    ...
    <HpBar enemy hp={state.enemyHp} previous={state.previousEnemyHp} max={DEMO_RULES.enemyMaxHp} phase={state.phase}/>
    ...
    <section className="bu-field" aria-label="单语灵战场" data-phase={state.phase} data-skill={state.selected ?? ''}>
      ...
      <div className="bu-combatant bu-ally" data-testid="active-spirit"><img src="/battle-ui/lange-cutout.png" alt="出战语灵：澜歌" draggable={false}/><div className="bu-nameplate"><span>澜歌</span><small>出战中</small></div></div>
      <div className="bu-combatant bu-enemy" data-testid="enemy-target"><div className="bu-enemy-token" aria-label="守门人立绘占位">魔</div><div className="bu-nameplate"><span>守门人</span><small>立绘待定</small></div></div>
      {state.phase === 'player' && state.damage > 0 && <div key={`${state.turn}-damage`} className="bu-float bu-damage bu-float-enemy">−{state.damage}<small>伤害</small></div>}
      ...
      {state.phase === 'player' && (state.weaken > 0 || state.mitigation > 0) && <div className="bu-effect-label">{state.weaken ? `下一击削弱 ${Math.round(state.weaken * 100)}%` : `下一击减伤 ${Math.round(state.mitigation * 100)}%`}</div>}
    ...
    <HpBar hp={state.playerHp} previous={state.previousPlayerHp} max={DEMO_RULES.playerMaxHp} phase={state.phase}/>
    ...
    <section className="bu-commands" aria-label="技能操作">
      ...
      <div className="t-acc" data-open={Boolean(skill)}>
        <div className="t-acc-panel"><div className="t-acc-panel-inner">
          <div id="bu-detail" className="bu-detail" hidden={!skill}><div><span className="bu-detail-eyebrow">技能详情</span><h2>{skill?.name}<small>{skill?.role}</small></h2><p>{skill ? skillDescription(skill) : ''}</p></div><button ref={castRef} className="bu-cast" disabled={!choosing || !skill} onClick={() => dispatch({ type: 'cast' })}>{choosing ? `施放「${skill?.name ?? ''}」` : '行动进行中'}<span aria-hidden="true"> →</span></button></div>
        </div></div>
      </div>
    ...
    <details className="bu-scope"><summary>样机范围与素材说明</summary><p>这里用于体验单语灵、血条、技能详情和回合反馈，不是微信小游戏正式版。技能演示沿用 V2 的澜歌三技能；生命和敌伤沿用 Phase A 的 48 / 60 / 8，仅作为界面验证数据，不代表正式平衡。敌方素材 PENDING_K3。背景与澜歌使用原图清理副本，原文件保留。</p></details>
  </main>;
}
