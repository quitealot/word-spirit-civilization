...
import { FIRE_SKILLS, FIRE_MOTION, initialFire, fireReducer, fireSkill, fireEnemy, fireBlocked, fireDamage, type FireId } from './fire-model';
...
export function FireBattle() {
  ...
  const motion = state.phase === 'player' && state.selected ? FIRE_MOTION[state.selected] : state.phase === 'enemy' ? BOSS_MOTION : null;
  ...
  return <main className="bu-shell jf-shell" style={{ '--bu-player-duration': `${motion?.durationMs ?? 2200}ms`, '--bu-player-impact': `${motion?.impactMs ?? 880}ms`, '--bu-boss-duration': '2400ms', '--bu-boss-impact': '960ms' } as CSSProperties}>
    ...
    <div ref={arena} className="bu-arena"><FireHp enemy hp={enemyHp} previous={state.previousEnemyHp}/>
      <section className="bu-field jf-field" aria-label="烬尾单语灵战场" data-phase={state.phase} data-skill={state.selected ?? ''} data-boss-skill={enemy.id} data-impact={pending ? 'pending' : 'shown'} data-reduced={reduced}>
        <div className="bu-stage" key={key} data-action={!!motion}><div className="bu-scene"/>
          <div className="bu-combatant bu-ally"><div className="bu-sprite jf-sprite"><img src={ART} alt="出战语灵：烬尾" draggable={false}/></div><div className="bu-nameplate"><span>烬尾</span><small>出战中</small></div></div>
          ...
          {state.phase === 'player' && !reduced && <div className={`jf-fx jf-fx-${state.selected}`} aria-hidden="true"><span className="jf-focus"/>{state.selected !== 'charge' && <TailFragment className="jf-fire-path"/>}<span className="jf-motes"><i/><i/><i/><i/><i/><i/></span>{!pending && state.selected !== 'charge' && <span className="jf-hit"/>}</div>}
    ...
    <footer className="bu-footer"><span>独立可玩预览 · 不写入存档</span><div><button aria-pressed={paused} onClick={() => setPaused(!paused)}>{paused ? '恢复自动' : '暂停自动'}</button>{paused && !choosing && !finished && <button onClick={() => pending ? setShown(key) : dispatch({ type: 'advance' })}>下一步</button>}<button disabled={!choosing && !finished} onClick={reset}>重开</button></div></footer>
