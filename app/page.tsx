...
export default function Home() {
  ...
  return <main className="p0-shell"><header className="p0-header"><div><span>《词灵》十关试玩</span><h1>雾中的回声</h1></div><div className="p0-header-actions"><div className="p0-stats"><b>{save.exploration}</b><span>探索力</span><b>{due}</b><span>到期复习</span></div><button onClick={reset}>重新开始</button></div></header>
    ...
    {trainingHub && <TrainingHubModal newCount={Math.max(0, (pack?.questions.length ?? 0) - (pack?.learned ?? 0))} dueCount={due} weakCount={weakWordIds.length} onClose={() => setTrainingHub(false)} onNew={() => { setTrainingHub(false); setLearning(true); }} onMaintenance={() => { setTrainingHub(false); setQuickTraining({ source: 'maintenance', wordIds: getDueWordIds(5) }); }} onTargeted={() => { setTrainingHub(false); setQuickTraining({ source: 'targeted', wordIds: weakWordIds.slice(0, BRIDGE_V1_RULES.training.targetedWordLimit) }); }} />}
    {quickTraining && <QuickTrainingModal source={quickTraining.source} wordIds={quickTraining.wordIds} onClose={() => setQuickTraining(null)} onAnswer={learningResult} onRecovered={wordId => { if (save.starter) recoverWeakness(weaknessEpisode(wordId), wordId, save.starter); }} />}
...
function TrainingHubModal({ newCount, dueCount: maintenanceCount, weakCount, onClose, onNew, onMaintenance, onTargeted }: { newCount: number; dueCount: number; weakCount: number; onClose: () => void; onNew: () => void; onMaintenance: () => void; onTargeted: () => void }) { return <div className="modal-backdrop"><section className="adventure-bridge training-hub"><button className="close" onClick={onClose}>×</button><span>训练</span><h2>这次想改善什么？</h2><div className="training-source-actions"><button disabled={newCount === 0} onClick={onNew}><b>今日引导</b><strong>{newCount}</strong><small>认识新的L1内容</small></button><button disabled={maintenanceCount === 0} onClick={onMaintenance}><b>共鸣维护</b><strong>{maintenanceCount}</strong><small>FSRS当前到期</small></button><button disabled={weakCount === 0} onClick={onTargeted}><b>针对训练</b><strong>{weakCount}</strong><small>最近实战暴露的问题</small></button></div></section></div>; }
function QuickTrainingModal({ source, wordIds, onClose, onAnswer, onRecovered }: { source: 'maintenance' | 'targeted'; wordIds: string[]; onClose: () => void; onAnswer: (correct: boolean, seen: boolean, question: LearningQuestion) => void; onRecovered: (wordId: string) => void }) {
  ...
  return <div className="modal-backdrop"><section className="learn-sheet quick-training"><button className="close" onClick={onClose}>×</button><span>{source === 'targeted' ? '针对训练' : '共鸣维护'}　{index + 1}/{wordIds.length}</span><h2>{question.word}</h2><i>{question.phonetic}</i><p>{question.prompt}</p><div>{question.choices.map(choice => <button key={choice} disabled={!!feedback} onClick={() => answer(choice)}>{choice}</button>)}</div>{feedback && <strong>{feedback}</strong>}</section></div>;
}
...
function LearningModal({ pack, onClose, onCancel = onClose, onAnswer }: { pack: ReturnType<typeof getCurrentLearningPack>; onClose: () => void; onCancel?: () => void; onAnswer: (correct: boolean, seen: boolean, question: LearningQuestion) => void }) {
...
function BattleModal({ episode, spirit, save, onClose, onAnswer, onAdventureCall, onStableSkill, onWeaknessRecovered, onSwap, onBossPhase, onWin }: BattleProps) {
  ...
  const [enemy, setEnemy] = useState(initialEnemy), [ally, setAlly] = useState(100), [defenseBuffer, setDefenseBuffer] = useState(0), [teamBattle, setTeamBattle] = useState<TeamBattleState>(makeTeam), [phase, setPhase] = useState<1 | 2 | 3>(1), [question, setQuestion] = useState<LearningQuestion | null>(null), [skill, setSkill] = useState(spirit.skills[0]), [teamTactic, setTeamTactic] = useState<'cover' | 'soften' | null>(null), [message, setMessage] = useState('[PENDING_K3]'), [wrongId, setWrongId] = useState<string>(), [defeated, setDefeated] = useState(false), [battleEvidence, setBattleEvidence] = useState<BattleEvidence[]>([]), [targetTraining, setTargetTraining] = useState(false), [targetTrainingDone, setTargetTrainingDone] = useState(false);
  ...
  function retry() { setEnemy(initialEnemy); setAlly(100); setDefenseBuffer(0); setTeamBattle(makeTeam()); setPhase(1); setQuestion(null); setSkill(spirit.skills[0]); setTeamTactic(null); setWrongId(undefined); setDefeated(false); setBattleEvidence([]); setTargetTraining(false); setTargetTrainingDone(false); setMessage('[PENDING_K3]'); outcome.current = 'active'; battleLog({ turn: 1, activeSpirit: spirit.name, enemyAttackKind: requiresSwap ? 'heavy' : 'normal', battleResult: 'start' }); }
  ...
  function answer(choice: string) {
    ...
    setBattleEvidence(current => [...current, { wordId: question.wordId, word: question.word, quality, skillName: teamTactic ? (teamTactic === 'cover' ? '覆护' : '缓冲') : skill.name, effectPercent, spiritId: activeSpiritId }]);
    setQuestion(null);
    if (requiresSwap) {
    ...
    if (next > 0) {
      ...
      setMessage(`${skill.name}${executionFeedback} · 造成${damage}伤害${generatedDefense ? ` · 防护${generatedDefense}` : ''}${recovery ? ` · 恢复${recovery}` : ''}${damageTaken ? ` · 受到反击${damageTaken}` : ''}`);
