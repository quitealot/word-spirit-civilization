...
import { clearGameSave, completeEp08ArenaSnapshot, completeEpisode, confirmEp06Companion, createEmptySave, ep1CheckpointAfterStarterChoice, getEpisodeExplorationGap, isEpisodeCompleted, isEpisodeUnlocked, loadGameSave, markEp10BossDefeated, recordEp07Swap, recordEp08Clue, recordEp09SkySilhouette, recordEp09TrackingAction, saveGameSave, setEp05Sightings, setEp06ApproachStage, setEp08MonumentFace, setEp09RareClueCount, setEp10BossPhase, type ApproachStage, type Ep1BondEvidence, type Ep1GuideOutcome, type GameSave, type StarterId, type TrackingSlotId } from './game/save';
...
export default function Home() {
  ...
  function recordBattleClear(ep: Episode) { update(s => { let next = s; if (ep === 5) next = { ...next, episodeState: { ...next.episodeState, ep05: { ...next.episodeState.ep05, battleCompleted: true } } }; if (ep === 7) next = { ...next, episodeState: { ...next.episodeState, ep07: { ...next.episodeState.ep07, battleCompleted: true } } }; if (ep === 10) next = markEp10BossDefeated(next); if (next.starter) next = grantBattleGrowth(next, next.starter, `ep${String(ep).padStart(2, '0')}`).save; if (ep === 3 && next.starter) next = grantResonanceMilestone(next, 'ep03.first_bond', [next.starter], 3); if (ep === 7 && next.starter) next = grantResonanceMilestone(next, 'ep07.teamwork', [next.starter, 'MIST_PORT_SPIRIT_01'], 3); if (ep === 10 && next.starter) next = grantResonanceMilestone(next, 'ep10.chapter_clear', [next.starter, 'MIST_PORT_SPIRIT_01'], 5); return next; }); }
